...
$sql = "SELECT s.id AS sale_id, s.plot_id, s.buyer_name AS client_name, s.buyer_phone AS phone, s.buyer_email AS email, s.agent_name, p.plot_number, p.status, e.name AS estate_name, s.amount, s.date_sold, s.payment_plan, s.deposit_doc, s.id_doc, s.kra_doc, s.passport_photo
        FROM prop_sales s
        JOIN prop_plots p ON p.id = s.plot_id
...
$page_title = 'Sold Plots - ProProperty';

ob_start();
?>
  <div class="top-bar">
    <h1>All Sold Plots</h1>
